...
- **Backend de IA — switch `AI_BACKEND`:** `studio` (default; Gemini API con `GEMINI_API_KEY`) o `vertex` (Vertex AI con `VERTEX_API_KEY`, key de GCP restringida a la Vertex AI API — el proyecto viene amarrado a la key, NO se pasa project/location al SDK). La decisión vive SOLO en `di/index.ts` (`useVertexBackend()` / `getBackendApiKey()` / `buildAiProvider()`); los providers son subclases delgadas de `GoogleGenAiProvider` y NUNCA leen env. Excepción: la voz (Live API + tokens efímeros v1alpha) usa SIEMPRE AI Studio — Vertex no tiene tokens efímeros.
