...
- **Skills de actualización: `assertHasChanges(changes)` antes de llamar al service** (`skills/skill.core.ts`). Sin eso, una actualización vacía es indistinguible de una exitosa —los services no escriben nada pero igual devuelven el registro completo y sin error— y el modelo le confirma al asesor un cambio que nunca ocurrió. Ya lo usan `update_reminder`, `update_contact` y `update_policy`; cualquier skill de update nueva debe hacerlo
