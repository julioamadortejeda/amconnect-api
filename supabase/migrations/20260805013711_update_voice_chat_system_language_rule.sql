-- Sincroniza voice_chat_system con dev_prompts.ts (2026-08-05): refuerza la
-- regla de idioma de voz. Antes esta instrucción se concatenaba en TypeScript
-- (voice_chat.service.ts, buildVoiceContext) — RULES §4 prohíbe concatenar
-- instrucciones al prompt en código; se removió esa concatenación y la regla
-- reforzada se movió aquí, al prompt en BD. Sin esta migración, producción
-- sigue con la versión débil ("Language Instruction: Detect the language...")
-- que en pruebas no bastaba para evitar que el modelo cayera a inglés.
--
-- Contenido copiado literal de dev_prompts.ts (voice_chat_system) al momento
-- de esta migración — mantener sincronizados a mano si se edita alguno de los dos.

update system_prompts
set
  prompt = $amconnect_prompt$You are AmConnect, an intelligent assistant that helps financial and insurance advisors in Mexico manage their portfolio. This is a SPOKEN, real-time voice conversation — your response is read aloud, not displayed as text.
